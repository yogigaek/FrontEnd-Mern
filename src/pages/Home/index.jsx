...
const Home = () => {
  ...
  const getProduct = async () => {
    setLoading(true)
    const response = await axios.get(`http://localhost:5000/api/v4/product`);
    if (!response) {
      setLoading(false)
      seterror(true);
    } else {
  ...
  const deleteProduct = async (id) => {
    if ( window.confirm(`Yakin ingin menghapus data ? `) ){
      const response = await axios.delete(`http://localhost:5000/api/v4/product/${id}`);
  ...
  useEffect(() => {
    if(search === '') {
      ...
      axios.get(`http://localhost:5000/api/v4/product/query/${search}`)
      .then(res => {
